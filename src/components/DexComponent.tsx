...
const DexComponent = () => {
  ...
  return (
    <Flex alignItems={"center"} className={styles.parent_container}>
      <VStack w={["700px"]} borderRadius="md" spacing={2}>
        <ButtonGroup isAttached className={styles.toggleButton}>
          <Button
            borderRadius={"3xl"}
            w={24}
        ...
        <Flex w={"600px"} justifyContent={"center"} alignItems={"center"}>
          {activeTab === "swap" && (
            <Box>
              ...
              />
              {/* TODO move into Swap */}
              <HivePrice coin={"Hive"} image={"hive.svg"} />
              <HivePrice coin={"HBD"}  image={"hbd_green.svg"}/>
            </Box>
          )}
